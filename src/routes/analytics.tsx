...
export const Route = createFileRoute("/analytics")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Track deliverability, engagement and revenue across channels.",
      },
    ],
  }),
  component: Analytics,
...
function Analytics() {
  ...
  const analyticsCards = {
    sent: funnel[0]?.value ?? 0,
    delivered: funnel[1]?.value ?? 0,
    opened: funnel[2]?.value ?? 0,
    clicked: funnel[3]?.value ?? 0,
    converted: funnel[4]?.value ?? 0,
  };
  ...
  const hasEngagement = engagementTrend.some(
    (d) => d.email > 0 || d.whatsapp > 0 || d.sms > 0 || d.push > 0,
  );
  const hasRevenue = revenueAttribution.some((r) => r.revenue > 0);
    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 space-y-6 p-4 md:p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <StatCard
            label="Delivered"
            value={formatNum(analyticsCards.delivered)}
            delta={4.9}
            icon={Inbox}
          />
          <StatCard
            label="Opened"
            value={formatNum(analyticsCards.opened)}
            delta={8.2}
            icon={MailOpen}
          />
          <StatCard
            label="Clicked"
            value={formatNum(analyticsCards.clicked)}
            delta={12.6}
            icon={MousePointerClick}
          />
          <StatCard
            label="Converted"
            value={formatNum(analyticsCards.converted)}
            delta={18.4}
            icon={CheckCircle2}
          />
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            ...
            <CardContent>
              <ChartShell
                isEmpty={!hasFunnel}
                empty="No communication events recorded yet. Launch a campaign to populate the funnel."
              >
          ...
          <Card className="border-primary/20 bg-gradient-to-br from-primary/5 via-card to-card">
            ...
            <CardContent className="space-y-3 text-sm">
              <Insight
                tone="positive"
                text="WhatsApp campaigns generated 32% higher engagement than email this month."
              />
              <Insight
                tone="warning"
                text="SMS conversion has dropped 8% — consider reducing send frequency."
              />
              <Insight
                tone="positive"
                text="Personalized subject lines lift open rate by 21% on email."
              />
              <Insight
                tone="info"
                text="Best time to send: Tuesday 10–11 AM for Bangalore segment."
              />
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <Card>
            ...
            <CardContent>
              <ChartShell isEmpty={!hasEngagement} empty="No engagement events in the last 7 days.">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
        ...
        <Card>
          ...
          <CardContent className="grid gap-6 md:grid-cols-[260px_1fr]">
            <ChartShell
              heightClass="h-56"
              isEmpty={!hasRevenue}
              empty="No revenue attribution data yet."
            >
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
            ...
            <div className="space-y-2">
              {revenueAttribution.length === 0 ? (
                <p className="text-sm text-muted-foreground">
                  No channel revenue breakdown available.
                </p>
              ) : (
