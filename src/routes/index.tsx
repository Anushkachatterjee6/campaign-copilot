...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Customer, campaign, and revenue overview for your AI-native CRM.",
      },
    ],
  }),
  component: Dashboard,
...
function Dashboard() {
  ...
  const revenueInfluenced =
    liveStats?.revenue_influenced ?? liveStats?.revenue_influenced_inr ?? null;
  ...
  const hasCampaignTrend = campaignTrend.some((d) => d.sent > 0 || d.opened > 0 || d.converted > 0);
  const hasChannelData = channelPerformance.some((d) => d.engagement > 0 || d.conversion > 0);
  const hasCustomerActivity = customerActivity.some((d) => d.active > 0 || d.new > 0);

  const aiRecommendations = [
    {
      id: 1,
      title: "Boost WhatsApp Engagement",
      impact: "High",
      detail: "WhatsApp campaigns have a 32% higher engagement rate. Focus more on this channel.",
    },
    {
      id: 2,
      title: "Optimize Send Times",
      impact: "Medium",
      detail: "Best time to send is Tuesday 10-11 AM.",
    },
  ];
      ...
      <main className="flex-1 space-y-6 p-4 md:p-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <StatCard
            ...
            value={statsLoading ? "…" : totalCustomers !== null ? formatNum(totalCustomers) : "—"}
            delta={4.2}
            icon={Users}
          />
          <StatCard
            label="Total Orders"
            value={statsLoading ? "…" : totalOrders !== null ? formatNum(totalOrders) : "—"}
            delta={6.8}
            icon={ShoppingCart}
          />
          <StatCard
            label="Active Campaigns"
            value={statsLoading ? "…" : activeCampaigns !== null ? String(activeCampaigns) : "—"}
            delta={2.1}
            icon={Megaphone}
          />
          <StatCard
            ...
            value={
              statsLoading ? "…" : revenueInfluenced !== null ? formatINR(revenueInfluenced) : "—"
            }
            delta={12.4}
            icon={IndianRupee}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            <CardHeader>
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <CardDescription>
                    Sent, opened, and converted over the last 7 days.
                  </CardDescription>
                </div>
            ...
            <CardContent>
              <ChartShell
                isEmpty={!chartsLoading && !hasCampaignTrend}
                empty="No campaign events in the last 7 days. Launch a campaign to see trends."
              >
          ...
          <Card>
            ...
            <CardContent>
              <ChartShell
                isEmpty={!chartsLoading && !hasChannelData}
                empty="No channel engagement data yet."
              >
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            ...
            <CardContent>
              <ChartShell
                isEmpty={!chartsLoading && !hasCustomerActivity}
                empty="No customer order activity in the selected period."
              >
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
              ...
              {aiRecommendations.map((r) => (
                <div key={r.id} className="rounded-lg border bg-card p-3">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="-ml-2 mt-2 h-7 gap-1 text-xs text-primary"
                    asChild
                  >
                    <Link to="/copilot">
                      Run with Copilot <ArrowRight className="h-3 w-3" />
                    </Link>
                  </Button>
                </div>
              ))}
            ...
            <CardContent className="px-0">
              {statsLoading ? (
                ...
                <p className="px-6 py-10 text-center text-sm text-muted-foreground">
                  No campaigns yet. Create one with the AI Copilot.
                </p>
              ) : (
                      ...
                      <div key={id} className="flex items-center gap-4 px-6 py-3">
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="mt-1 flex items-center gap-2">
                            <ChannelBadge
                              channel={channelDisplay as "Email" | "WhatsApp" | "SMS" | "Push"}
                            />
                            <StatusBadge
                              status={
                                statusDisplay as
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatNum(s.customer_count)} customers
                    </p>
                  </div>
                  <span
